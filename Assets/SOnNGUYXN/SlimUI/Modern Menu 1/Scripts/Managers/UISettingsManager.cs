...
namespace SlimUI.ModernMenu
{
	public class UISettingsManager : MonoBehaviour
	{
		...
		public AudioMixer audioMixer; // <- Kéo AudioMixer có exposed parameter tên là "MusicBackground"
									  // sliders
		...
		public void Start()
		{
			...
			musicSlider.GetComponent<Slider>().value = savedVolume;
		...
		{
			float sliderValue = musicSlider.GetComponent<Slider>().value;
		...
		{
			// Convert 0.0001 ~ 1 to -80 ~ 0 dB
			float volumeDb = Mathf.Log10(Mathf.Clamp(value, 0.0001f, 1f)) * 20f;
			audioMixer.SetFloat("MyExposedParam", volumeDb);  // Đúng tên exposed param bạn vừa đặt
		}
